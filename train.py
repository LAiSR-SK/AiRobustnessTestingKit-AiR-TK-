...
parser.add_argument('--save-freq', '-s', default=1, type=int, metavar='N',
                    help='save frequency')
parser.add_argument('--swa-warmup', default=50, type=int, metavar='N',
                    help='save frequency')
...
def train(args, model, device, train_loader, optimizer, epoch, attacks, ds_name, f, swa_model, swa_scheduler, scheduler):
    """
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        loss = adt_va_loss(model, data, target, attack, ds_name, optimizer, device)
...
def main_va_epochs(ds_name, mod_name, clean_epochs=0):
    ...
    torch.save(model.state_dict(),
               os.path.join(model_dir, 'model-adt-va-{}-{}-start.pt'.format(ds_name, mod_name)))
    torch.save(optimizer.state_dict(),
               os.path.join(model_dir, 'opt-adt-va-{}-{}-start.tar'.format(ds_name, mod_name)))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train(args, model, device, train_loader, optimizer, epoch, attacks, ds_name, f, swa_model, swa_scheduler, scheduler)
        ...
        if args.swa:
            if epoch > args.swa_warmup:
                eval_clean(swa_model, device, train_loader, 'train', ds_name, f)
        ...
        if epoch % args.save_freq == 0:
            torch.save(model.state_dict(),
                       os.path.join(model_dir, 'model-adt-va-{}-{}-epoch{}.pt'.format(ds_name, mod_name, epoch)))
            torch.save(swa_model.state_dict(),
                       os.path.join(model_dir, 'swa-model-adt-va-{}-{}-epoch{}.pt'.format(ds_name, mod_name, epoch)))
            torch.save(optimizer.state_dict(),
                       os.path.join(model_dir, 'opt-adt-va-{}-{}-epoch{}.tar'.format(ds_name, mod_name, epoch)))
    ...
    if args.swa:
        ...
        trained_model.load_state_dict(
            torch.load('saved-models/swa-model-adt-va-{}-{}-epoch{}.pt'.format(ds_name, mod_name, args.epochs)))
    else:
        trained_model.load_state_dict(
            torch.load('saved-models/model-adt-va-{}-{}-epoch{}.pt'.format(ds_name, mod_name, args.epochs)))
