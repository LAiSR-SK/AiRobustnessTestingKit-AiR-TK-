    ...
    def earlystop(
        ...
        if rand_init:
            if randominit_type == "normal_distribution_randominit":
                iter_adv = (
                    data.detach() + 0.001 * torch.randn(data.shape).to(self.device).detach()
                )
        ...
        while K > 0:
            ...
            if len(output_index) != 0:
                if len(output_target) == 0:
                    ...
                    output_adv = (
                        iter_adv[output_index].reshape(-1, 3, 32, 32).to(self.device)
                    )
                    output_natural = (
                        iter_clean_data[output_index].reshape(-1, 3, 32, 32).to(self.device)
                    )
                    output_target = iter_target[output_index].reshape(-1).to(self.device)
                else:
                    # incorrect adv data should not keep iterated
            ...
            with torch.enable_grad():
                ...
                if loss_fn == "kl":
                    criterion_kl = nn.KLDivLoss(size_average=False).to(self.device)
            ...
            if len(iter_index) != 0:
                ...
                iter_adv = (
                    iter_adv.detach()
                    + eta
                    + omega * torch.randn(iter_adv.shape).detach().to(self.device)
                )
                iter_adv = torch.min(
        ...
        if len(output_target) == 0:
            ...
            output_natural = iter_clean_data.reshape(-1, 3, 32, 32).to(self.device)
        else:
            output_adv = torch.cat(
