...
def get_args():
    ...
    )  # currently set to WideResNet, could be ResNet18
    ...
    parser.add_argument(
        "--attack_iters_types", type=int, nargs="*", default=range(3, 14)
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--policy_optimizer",
        default="SGD_without_momentum",
        type=str,
        help="policy_optimizer",
    )
    parser.add_argument(
        "--factor", default=0.7, type=float, help="Label Smoothing"
    )
...
def main_lasat10():
    ...
    torch.save(
        target_model.state_dict(),
        os.path.join(
            model_dir, "model-lasat-{}-{}.pt".format("cifar10", "wideres34")
        ),
    )
