...
def ImageNet_get_loaders(dir_, batch_size):
    ...
    num_workers = 0
    image_datasets = {
        x: datasets.ImageFolder(os.path.join(dir_, x), data_transforms[x])
...
def evaluate_pgd(
    test_loader, model, attack_iters, restarts, epsilon=(8 / 255.0) / std
):
    print(epsilon)
    alpha = (2 / 255.0) / std
    pgd_loss = 0
...
def evaluate_pgd_cw_cifar100(test_loader, model, attack_iters, restarts):
    ...
    for i, (X, y) in enumerate(test_loader):
        X, y = X.cuda(), y.cuda()
        pgd_delta = cw_Linf_attack_cifar100(
            model,
            X,
            y,
            epsilon,
