...
class ResNet_Strategy(nn.Module):
    def __init__(self, block, num_blocks, args):
        ...
        self.Attack_method = nn.Linear(
            512 * block.expansion, len(args.attack_types)
        )  # 所用攻击方式的个数 1
        self.Attack_epsilon = nn.Linear(
            512 * block.expansion, len(args.epsilon_types)
        )  # 攻击强度 11
        self.Attack_iters = nn.Linear(
            512 * block.expansion, len(args.attack_iters_types)
        )  # 迭代次数 11
        self.Attack_step_size = nn.Linear(
            512 * block.expansion, len(args.step_size_types)
        )  # 步长
...
def get_args():
    ...
    parser.add_argument(
        "--target_model_lr", default=0.2, type=float, help="learning rate"
    )
    ...
    parser.add_argument(
        "--attack_iters_types", type=list, default=range(1, 10)
    )
