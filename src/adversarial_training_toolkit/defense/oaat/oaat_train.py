...
# import wandb
from adversarial_training_toolkit.defense.oaat.autoaugment import CIFAR10Policy
from adversarial_training_toolkit.defense.oaat.defaults import use_default
from adversarial_training_toolkit.defense.oaat.perceptual_advex_perceptual_attack_adv import (
    get_lpips_model,
    get_lpips_model_100classes,
)

# from utils import Bar, Logger, AverageMeter, accuracy
from adversarial_training_toolkit.defense.oaat.utils import (
    AverageMeter,
    Logger,
    accuracy,
)
...
from adversarial_training_toolkit.defense.oaat.utils_awp import TradesAWP
from adversarial_training_toolkit.defesnse.oaat.perceptual_advex_distances import (
    LPIPSDistance,
)
...
parser = argparse.ArgumentParser(
    description="PyTorch OAAT Adversarial Training"
)
parser.add_argument(
    "--arch",
    type=str,
    default="WideResNet34",
    choices=["ResNet18", "PreActResNet18", "WideResNet34"],
)
parser.add_argument(
    "--batch-size",
    type=int,
...
parser.add_argument(
    "--data-path", type=str, default="../data", help="where is the dataset"
)
parser.add_argument(
    "--weight-decay", "--wd", default=3e-4, type=float, metavar="W"
)
parser.add_argument(
    "--lr", type=float, default=0.2, metavar="LR", help="learning rate"
)
parser.add_argument(
...
parser.add_argument(
    "--epsilon", default=16 / 255, type=float, help="perturbation"
)
parser.add_argument(
    "--beta",
    default=2.0,
...
parser.add_argument(
    "--exp_name", default="OAAT", help="name of the method used for training"
)
parser.add_argument(
    "--beta_final",
    default=3,
...
parser.add_argument(
    ...
    type=float,
    nargs="*",
    default=[0.995, 0.9996, 0.9998],
    help="The tau values for SWA",
)
parser.add_argument(
    "--label_smoothing",
    default=0,
...
def perturb_input(
    ...
    if distance == "l_inf":
        x_adv = (
            x_natural.detach()
            + torch.FloatTensor(np.random.uniform(-noi, noi, x_natural.shape))
            .cuda()
            .detach()
        )
...
def perturb_input_lpips(
    ...
    noise = torch.FloatTensor(
        np.random.uniform(-eps, eps, (B, C, H, W))
    ).cuda()
    ...
    for step in range(steps):
        ...
        if args.use_CE == 1:
            cost = torch.nn.CrossEntropyLoss()(
                out, target.cuda()
            ) - lpips_weight * torch.mean(lpips_distance(y_imgs, img))
        else:
            cost = F.kl_div(
                F.log_softmax(model(img), dim=1),
        ...
        per[:, 0, :, :] = (eps_iter * (bounds[0, 1] - bounds[0, 0])) * per[
            :, 0, :, :
        ]
        if per.size(1) > 1:
            per[:, 1, :, :] = (eps_iter * (bounds[1, 1] - bounds[1, 0])) * per[
                :, 1, :, :
            ]
            per[:, 2, :, :] = (eps_iter * (bounds[2, 1] - bounds[2, 0])) * per[
                :, 2, :, :
            ]
        #  ascent
        ...
        img[:, 0, :, :] = torch.clamp(
            adv[:, 0, :, :], bounds[0, 0], bounds[0, 1]
        )
        if per.size(1) > 1:
            img[:, 1, :, :] = torch.clamp(
                adv[:, 1, :, :], bounds[1, 0], bounds[1, 1]
            )
            img[:, 2, :, :] = torch.clamp(
                adv[:, 2, :, :], bounds[2, 0], bounds[2, 1]
            )
        img = img.data
        noise = img - data
        noise = torch.clamp(noise, -eps, eps)
...
def train_lpips_alt(
    ...
    for batch_idx, (data, data_auto, data_test, target, index) in enumerate(
        ...
        if batch_idx % 2 == 0:
            x_adv_vareps = torch.min(
                torch.max(x_adv, x_natural - vareps), x_natural + vareps
            )
            logits_adv_vareps = model(x_adv_vareps)
            loss_robust = F.kl_div(
                F.log_softmax(logits_adv_vareps, dim=1),
                alpha * F.softmax(logits_clean, dim=1)
                + (1 - alpha) * F.softmax(logits_adv, dim=1),
                reduction="batchmean",
            )
        else:
            loss_robust = F.kl_div(
...
def adjust_learning_rate_cosine(optimizer, epoch, args):
    if args.OAAT_warmup == 1:
        lr = (
            args.lr
            * 0.5
            * (1 + np.cos((epoch - 11) / (args.epochs - 10) * np.pi))
        )
    else:
...
def main_oaat(ds_name):
    if ds_name == "CIFAR100":
        num_classes = 100
    elif ds_name == "CIFAR10" or args.data == "SVHN":
        num_classes = 10
    ...
    if args.auto == 1:
        ...
        transform_train_main_SVHN = transforms.Compose(
            [
                policy,
                transforms.ToTensor(),
            ]
    ...
    transform_train_auto_SVHN = transforms.Compose(
        [
            policy,
            transforms.ToTensor(),
        ]
    ...
    if ds_name == "CIFAR100":
        trainset = Aug_loader_cifar100(
            root=args.data_path,
            train=True,
            transform=[
                transform_train_main,
                transform_train_auto,
                transform_test,
            ],
            download=True,
        )
    else:
        trainset = Aug_loader_cifar10(
            root=args.data_path,
            train=True,
            transform=[
                transform_train_main,
    ...
    valset = getattr(datasets, ds_name)(
        root=args.data_path,
        train=True,
        download=True,
        transform=transform_test,
    )
    testset = getattr(datasets, ds_name)(
        root=args.data_path,
        train=False,
        download=True,
        transform=transform_test,
    ...
    proxy = nn.DataParallel(ResNet18(num_classes=num_classes)).to(device)
    ...
    for i in range(len(tau_list)):
        model_tau = ResNet18(num_classes=num_classes)
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        if vareps > args.alternate_iter_eps:
            ...
            if ds_name == "CIFAR100":
                lpips_model = get_lpips_model_100classes(
                    args.arch,
            ...
            lpips_weight = (
                args.lpips_weight
                * (epoch - int(3 * args.epochs / 4))
                / (args.epochs - int(3 * args.epochs / 4))
            )
            args.beta = beta_initial + (args.beta_final - beta_initial) * (
                epoch - int(3 * args.epochs / 4)
            ) / (args.epochs - int(3 * args.epochs / 4))
            lpips_model = lpips_model.cuda()
            lpips_distance = LPIPSDistance(lpips_model)
            adv_loss, adv_acc, clean_loss, clean_acc, exp_avgs = (
                train_lpips_alt(
                    model,
                    train_loader,
                    optimizer,
            ...
            adv_loss, adv_acc, clean_loss, clean_acc, exp_avgs = train(
                model,
                train_loader,
                optimizer,
                epoch,
        ...
        logger.append(
            [lr, adv_loss, clean_loss, val_loss, adv_acc, clean_acc, val_acc]
        )
        ...
        if epoch > args.swa_save_epoch - 1:
            for idx, start_ep, tau, new_state_dict in zip(
                range(len(tau_list)), start_wa, tau_list, exp_avgs
            ):
                if start_ep <= epoch:
                    torch.save(
                        new_state_dict,
