...
from adversarial_training_toolkit.defense.atawp.at_awp_utils_awp import (
    AdvWeightPerturb,
)
...
def get_args():
    ...
    parser.add_argument(
        "--attack", default="pgd", type=str, choices=["pgd", "none"]
    )
...
def main():
    ...
    if args.model == "PreActResNet18":
        ...
        model = WideResNet(
            34, 100, widen_factor=args.width_factor, dropRate=0.0
        )
        proxy = WideResNet(
            34, 100, widen_factor=args.width_factor, dropRate=0.0
        )
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if (epoch + 1) % args.chkpt_iters == 0 or epoch + 1 == epochs:
            ...
            torch.save(
                opt.state_dict(), os.path.join(args.fname, f"opt_{epoch}.pth")
            )
...
def main_atawp100():
    ...
    torch.save(
        model.state_dict(),
        os.path.join(
            model_dir, "model-at-awp-{}-{}.pt".format("cifar100", "res18")
        ),
    )
